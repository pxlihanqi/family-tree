...
export function RichTextEditor({ id, value, onChange, maxLength = 500, className, placeholder }: RichTextEditorProps) {
  ...
  const initialValue = useMemo(() => {
    try {
      ...
      // 确保返回的是一个有效的Slate元素数组
      if (Array.isArray(parsedValue)) {
        // 验证数组中的每个元素是否是有效的Slate元素
        const isValidSlateArray = parsedValue.every(item => 
          typeof item === 'object' && 
          item !== null && 
      ...
      // 如果解析结果不是有效的Slate数组，将其作为纯文本处理
      return [{ type: 'paragraph', children: [{ text: stringValue }] }];
    } catch (e) {
      // Fallback for plain text legacy data or parsing errors
      return [{ type: 'paragraph', children: [{ text: String(value) }] }];
    }
  }, [value]) // 依赖value，确保value变化时重新计算
  ...
  const textContent = Array.isArray(internalValue) ? internalValue.map(n => Node.string(n)).join('\n') : ''
  const currentLength = textContent.length

  const handleChange = (newValue: Descendant[]) => {
    setInternalValue(newValue)
    const newTextContent = Array.isArray(newValue) ? newValue.map(n => Node.string(n)).join('\n') : ''
  ...
  return (
    <div className={cn("flex flex-col gap-2 rounded-md border p-2 overflow-x-hidden", className)}>
      <Slate editor={editor} initialValue={initialValue} onChange={handleChange}>
        ...
        <div className="min-h-[150px] overflow-x-hidden">
            <Editable
            renderElement={renderElement}
            renderLeaf={renderLeaf}
            placeholder={placeholder || "请输入生平事迹..."}
            spellCheck={false}
            onKeyDown={handleKeyDown}
            id={id}
            className="min-h-[150px] outline-none whitespace-pre-wrap break-words [overflow-wrap:anywhere]"
            />
        </div>
      </Slate>
