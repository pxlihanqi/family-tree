...
export function RichTextViewer({ value, className, animate = false }: RichTextViewerProps) {
  ...
  const initialValue = useMemo(() => {
    ...
    if (!value) {
      ...
      try {
        ...
        // 确保nodes是一个数组
        nodes = Array.isArray(parsedValue) ? parsedValue : [{ type: 'paragraph', children: [{ text: value }] }];
      } catch (e) {
  ...
  return (
    <div
      className={cn(
        "prose dark:prose-invert max-w-none overflow-x-hidden break-words [overflow-wrap:anywhere] [&_*]:max-w-full [&_p]:break-words [&_p]:[overflow-wrap:anywhere] [&_li]:break-words [&_li]:[overflow-wrap:anywhere] [&_blockquote]:break-words [&_blockquote]:[overflow-wrap:anywhere]",
        className
      )}
    >
      <Slate editor={editor} initialValue={initialValue}>
        <Editable
          ...
          className="outline-none whitespace-pre-wrap break-words [overflow-wrap:anywhere]"
        />
      </Slate>
    </div>
  )
