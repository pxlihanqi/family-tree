...
export async function fetchAllMembersForCsv(): Promise<{
  data: {
    id: number;
    name: string;
    generation: number | null;
  ...
  const { data, error } = await supabase
    .from("family_members")
    .select("id, name, generation, sibling_order, father_id, gender, official_position, is_alive, spouse, spouse_id, remarks, birthday, death_date, burial_place, residence_place, contact, avatar, updated_at")
    .order("generation", { ascending: true })
    .order("sibling_order", { ascending: true });
